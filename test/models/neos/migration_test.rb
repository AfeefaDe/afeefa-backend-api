...
module Neos
  class MigrationTest < ActiveSupport::TestCase
    ...
    should 'not annotate gone events' do
      event_relation =
        Neos::Event.where.not(datefrom: [nil, '']).order(:created).limit(1)
    ...
    should 'annotate empty dateto at event' do
      # Neos::Migration.unstub(:puts)
      event_relation =
        Neos::Event.where(datefrom: [nil, '']).order(:created).limit(1)
      event = event_relation.first
      assert event.update(published: true)
      Neos::Event.stubs(:where).returns(event_relation)
      ...
      assert_equal event.entry_id, new_event.legacy_entry_id
      assert new_event.in?(::Event.past)
      assert new_event.annotations.any?
      assert Annotation.where(entry_id: new_event.id, entry_type: 'Event').any?
      assert 'Start-Datum fehlt'.in?(new_event.annotations.map(&:detail))
    end
