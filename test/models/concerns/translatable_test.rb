...
class TranslatableTest < ActiveSupport::TestCase
  ...
  test 'create translation on facet_item create' do
    ...
    assert facet_item.save, facet_item.errors.inspect
  end
  ...
  test 'create translation on navigation_item create' do
    ...
    # needed for validation of nav item
    assert navigation_item.navigation.save
    assert navigation_item.navigation.id
    ...
    assert navigation_item.save, navigation_item.errors.inspect
  end
  ...
  [:orga, :event, :offer].each do |entry_factory|
    test "create fapi cache job on entry create for #{entry_factory}" do
  ...
  [:orga, :event, :offer].each do |entry_factory|
    test "create fapi cache job on entry update for #{entry_factory}" do
  ...
  [:orga, :event, :offer].each do |entry_factory|
    test "create fapi cache job on entry delete for #{entry_factory}" do
      entry = create(entry_factory)
