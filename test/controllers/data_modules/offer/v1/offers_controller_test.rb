...
class DataModules::Offer::V1::OffersControllerTest < ActionController::TestCase
  ...
  test 'create offer without owner' do
  ...
  test 'allow to create offer with existing title' do
  ...
  test 'create offer with owners' do
    actor = create(:orga)
  ...
  test 'create offer with owners and link contact of first owner' do
    actor = create(:orga)
  ...
  test 'raise exception if create offer with wrong actor' do
    assert_no_difference -> { DataModules::Offer::OfferOwner.count } do
  ...
  test 'raise exception if create offer with missing data' do
    assert_no_difference -> { DataModules::Offer::OfferOwner.count } do
      assert_no_difference -> { DataModules::Offer::Offer.count } do
        post :create, params: {}
        assert_response :unprocessable_entity
  ...
  test 'throw error if updating nonexistant offer' do
    patch :update, params: { id: 1123, titel: 'Neuer Name für Angebot' }
    assert_response :not_found
  end

  test 'delete offer' do
    actor = create(:orga)
    offer = create(:offer, actors: [actor.id])
  ...
  test 'throw error if deleting nonexisting offer' do
    delete :destroy, params: { id: 123 }
    assert_response :not_found
  end

  test 'deliver owners with different detail granularity' do
    actor = create(:orga)
    offer = create(:offer, actors: [actor.id])
  ...
  test 'deliver attributes and relations in show and index' do
    actor = create(:orga)
    offer = create(:offer, actors: [actor.id])
  ...
  test 'throw error on linking nonexisting owner' do
    owner = create(:orga)
    offer = create(:offer)
  ...
  test 'throw error on linking owner of different area' do
    owner = create(:orga)
  ...
  test 'convert actor and its relations to offer' do
