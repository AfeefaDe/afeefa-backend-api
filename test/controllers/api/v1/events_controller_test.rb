  ...
  test 'get title filtered list for events' do
    user = create(:user)
    orga = create(:orga)
  ...
  test 'show legacy events without date_start in past events' do
    user = create(:user)
    orga = create(:orga)
  ...
  test 'get events filtered for start and end' do
    user = create(:user)
    orga = create(:orga)
  ...
  test 'get events filtered for start and end of given orga' do
    user = create(:user)
    orga = create(:orga)
  ...
  test 'ensure creator for event on create' do
  ...
  test 'ignore given creator for event' do
    params = parse_json_file(file: 'create_event_without_orga.json') do |payload|
      payload.gsub!('<annotation_category_id_1>', AnnotationCategory.first.id.to_s)
      payload.gsub!('<annotation_category_id_2>', AnnotationCategory.second.id.to_s)
    end
    ...
    test 'get show event with time_start and time_end' do
      create_event
    ...
    test 'deliver hosts with different detail granularity' do
      create_event
    ...
    test 'deliver different attributes and relations when show or index' do
      create_event
  ...
  test 'I want to create a new event' do
    orga = create(:orga)
    params = parse_json_file(file: 'create_event_without_orga.json') do |payload|
      payload.gsub!('<annotation_category_id_1>', AnnotationCategory.first.id.to_s)
      payload.gsub!('<annotation_category_id_2>', AnnotationCategory.second.id.to_s)
  ...
  test 'allow to create event with existing title' do
  ...
  test 'create event with host' do
    actor = create(:orga)
  ...
  test 'create event with host and link contact of first host' do
    actor = create(:orga)
  ...
  test 'An event should only change allowed states' do
  ...
  test 'ignore given state on event create' do
    assert_difference 'Event.count' do
      params = parse_json_file(file: 'create_event_without_orga.json') do |payload|
        payload.gsub!('<annotation_category_id_1>', AnnotationCategory.first.id.to_s)
        payload.gsub!('<annotation_category_id_2>', AnnotationCategory.second.id.to_s)
  ...
  test 'set default orga on event create without orga relation' do
    params = parse_json_file(file: 'create_event_without_orga.json') do |payload|
  ...
  test 'update event without sub_category' do
  ...
  test 'update an event without creator and set the creator automatically' do
    assert event = create(:event)
    event.creator = nil
  ...
  test 'throw error on linking nonexisting host' do
    host = create(:orga)
    event = create(:event)
  ...
  test 'throw error on linking host of different area' do
    host = create(:orga)
