...
class Api::V1::EventsControllerTest < ActionController::TestCase
  ...
  context 'as authorized user' do
    ...
    #               #   type: 'events',
    #               #   attributes: {
    #               #     title: 'some titlejokjkljlkjkljklj',
    #               #     description: 'some descriptionasfdsafsdfsdfdsf',
    ...
    should 'test123' do
      params = parse_json_file(file: 'create_event.json') do |payload|
        payload.gsub!('<category_id>', Category.main_categories.first.id.to_s)
      end
      post :create, params: params
      assert_response :created, response.body
      params =
        { id: Event.last.id }.merge(
          parse_json_file(file: 'update_event.json') do |payload|
            payload.gsub!('<category_id>', Category.main_categories.first.id.to_s)
            payload.gsub!('<contact_info_id>', Event.last.contact_infos.first.id.to_s)
            payload.gsub!('<location_id>', Event.last.locations.first.id.to_s)
            payload.gsub!('<event_id>', Event.last.id.to_s)
          end
        )
    ...
    # end
    #
    # should 'not soft destroy event with associated sub_event' do
    #   assert @event = create(:event)
