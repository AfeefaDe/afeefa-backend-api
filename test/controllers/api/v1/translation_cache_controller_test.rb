...
class Api::V1::TranslationCacheControllerTest < ActionController::TestCase
  ...
  test 'get status 401 for missing phraseapp webhook token' do
    post :phraseapp_webhook
    assert_response :unauthorized
  end

  test 'get status 401 for wrong phraseapp webhook token' do
    post :phraseapp_webhook, params: { token: '123' }
    assert_response :unauthorized
  end

  test 'get status 200 for valid phraseapp webhook token' do
    @controller.expects(:phraseapp_webhook)
    post :phraseapp_webhook, params: { token: Settings.phraseapp.webhook_api_token }
    assert_response :no_content
  end

  test 'start cache job upon created translation' do
    orga = create(:orga)
  ...
  test 'start cache job upon created translation with offer description' do
  ...
  test 'start cache job upon updated translation' do
    orga = create(:orga)
  ...
  test 'start cache job upon updated translation with event' do
    event = create(:event)
  ...
  test 'start cache job upon updated translation with offer description' do
    offer = create(:offer)
  ...
  test 'start cache job upon updated translation with facet_item' do
  ...
  test 'remove cache record if all values are empty' do
    event = create(:event)
  ...
  test 'start cache job upon updated translation with navigation_item' do
  ...
  test 'get last updated timestamp' do
    get :index
