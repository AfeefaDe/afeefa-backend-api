...
class Api::V1::AnnotationsControllerTest < ActionController::TestCase
  ...
  [:orga, :event, :offer].each do |entry_factory|
    test "get annotations for #{entry_factory}" do
    ...
    test "create annotation for #{entry_factory}" do
      entry = create(entry_factory)
    ...
    test "raise error if create annotation fails for #{entry_factory}" do
      entry = create(entry_factory)
    ...
    test "raise error if update annotation fails for #{entry_factory}" do
      entry = create(entry_factory)
    ...
    test "delete annotation for #{entry_factory}" do
      entry = create(entry_factory)
    ...
    test "raise error if delete annotation fails for #{entry_factory}" do
      entry = create(entry_factory)
