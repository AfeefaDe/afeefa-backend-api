module ActsAsFacetItemControllerTest
  ...
  included do
    ...
    test 'set the right parent_id' do
      root = create_root_with_items_and_sub_items
    ...
    test 'not deliver owners multiple times if also added to sub items' do
      root = create_root_with_items_and_sub_items
    ...
    test 'update item with new parent' do
      root = create_root_with_items_and_sub_items
      parent = get_root_items(root).select { |item| item.sub_items.count > 0 }.first
      sub_item = parent.sub_items.first
    ...
    test 'throw error on update item with wrong params' do
      root = create_root_with_items
      item = get_root_items(root).first
    ...
    test 'throw error on link item again' do
      root = create_root_with_items
      item = get_root_items(root).first
      orga = create(:orga)
    ...
    test 'link owners of multiple types with item' do
      root = create_root_with_items
      item = get_root_items(root).first
      orga = create(:orga)
      event = create(:event)
    ...
    test 'unlink owner from item' do
      root = create_root_with_items
      item = get_root_items(root).first
      orga = create(:orga)
      item.link_owner(orga)
    ...
    test 'throw error on unlink facet item again' do
      root = create_root_with_items
      item = get_root_items(root).first
      orga = create(:orga)
    ...
    test 'link multiple owners with facet item' do
      root = create_root
    ...
    test 'call item.link_owner on link multiple owners' do
      root = create_root_with_items
      item = get_root_items(root).first
    ...
    test 'link owners of multiple types with facet item' do
      root = create_root
      item = create_item_with_root(root)
      orga = create(:orga)
      orga2 = create(:orga, title: 'another orga')
    ...
    test 'not fail if linking multiple owners fails for one owner with already existing association' do
      root = create_root
      item = create_item_with_root(root)
      orga = create(:orga)
      orga2 = create(:orga, title: 'another orga')
    ...
    test 'throw error if linking multiple owners fails for one owner which does not exist' do
      root = create_root
      item = create_item_with_root(root)
      orga = create(:orga)
    ...
    test 'throw error if linking multiple owners fails for all owners' do
      root = create_root
      item = create_item_with_root(root)
      orga = create(:orga)
      item.link_owner(orga)
    ...
    test 'unlink multiple owners from facet item' do
      root = create_root
      item = create_item_with_root(root)
      orga = create(:orga)
      orga2 = create(:orga, title: 'another orga')
    ...
    test 'call item.unlink_owner on link multiple owners' do
      root = create_root_with_items
      item = get_root_items(root).first
    ...
    test 'not fail if unlinking multiple owners fails for one owner without existing association' do
      root = create_root_with_items
      item = get_root_items(root).first
    ...
    test 'throw error if unlinking multiple owners fails for one nonexisting owner' do
      root = create_root_with_items
      item = get_root_items(root).first
    ...
    test 'throw error if unlinking multiple owners fails for all owners' do
      root = create_root_with_items
      item = get_root_items(root).first
