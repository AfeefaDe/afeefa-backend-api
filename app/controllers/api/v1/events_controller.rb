          ...
          case filter_criterion.to_sym
            when :upcoming
              ...
              objects.
                ...
                where('date_start > ?', now).or(objects.where('date_start = ?', now)).or(
		          objects.
					where('date_end > ?', now)
				)
            when :past
              ...
              objects.
                where(date_end: nil).where.not(date_start: nil).where.not(date_start: '').
                where('date_start < ?', now).or(
		          objects.
		            where(date_end: '').where.not(date_start: nil).where.not(date_start: '').
