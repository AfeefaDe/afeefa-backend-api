class Api::V1::TodosController < Api::V1::EntriesController
  ...
  def index
    ...
    todos = entries.map do |item|
      ...
      # item_hash =
      #   if item.is_a?(Event)
      #     EventSerializer.new(item, params).serializable_hash[:data]
      #   elsif item.is_a?(Orga)
      #     OrgaSerializer.new(item, params).serializable_hash[:data]
      #   elsif item.is_a?(DataModules::Offer::Offer)
      #     OfferSerializer.new(item, params).serializable_hash[:data]
      #   end
